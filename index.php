...
$posts = [];
try {
    $posts = db()->query("SELECT p.id,p.title,p.summary,p.thumbnail,p.published_at,c.name AS category_name,u.full_name AS author_name FROM posts p JOIN categories c ON c.id=p.category_id JOIN users u ON u.id=p.author_id WHERE p.status='published' AND c.status='active' ORDER BY COALESCE(p.published_at,p.updated_at) DESC LIMIT 4")->fetchAll();
...
<section id="articles" class="home-section"><div class="site-shell"><div class="section-title"><h2>Bài viết và hướng dẫn</h2><a href="dang-phat-trien.php?feature=danh-sach-bai">Xem tất cả →</a></div><div class="article-grid">
<?php foreach ($posts as $index=>$post): $isReal=isset($post['id']); $image=!empty($post['thumbnail']) ? 'assets/uploads/'.$post['thumbnail'] : 'assets/images/figma/'.($post['image'] ?? $demoPosts[$index]['image']); $detailUrl=$isReal ? 'bai-viet.php?id='.(int)$post['id'] : 'dang-phat-trien.php?feature=bai-minh-hoa'; ?><article class="article-card"><a class="article-image-link" href="<?= e($detailUrl) ?>"><img src="<?= e($image) ?>" alt=""></a><div><div class="article-meta"><span><?= e($post['category_name']) ?></span><span><?= isset($post['published_at']) && $post['published_at'] ? e(date('d/m/Y',strtotime($post['published_at']))) : '12/05/2026' ?></span></div><h3><a href="<?= e($detailUrl) ?>"><?= e($post['title']) ?></a></h3><p><?= e($post['summary']) ?></p><div class="article-actions"><small>◷ 5 phút đọc</small><a class="save-button" href="<?= empty($_SESSION['user']) ? 'dang-nhap.php' : 'dang-phat-trien.php?feature=impact-box' ?>" aria-label="Lưu bài viết vào Impact Box" title="Lưu bài viết">♡</a></div></div></article><?php endforeach; ?>
</div></div></section>

<section id="topics" class="topic-section"><div class="site-shell topic-grid"><article><img src="assets/images/figma/icon-study.png" alt=""><div><h3>Học tập</h3><p>Lịch học, học phần, hướng dẫn học tập và tài liệu</p><a href="dang-phat-trien.php?feature=hoc-tap">Khám phá →</a></div></article><article><img src="assets/images/figma/icon-opportunity.svg" alt=""><div><h3>Cơ hội</h3><p>Học bổng, tuyển dụng, thực tập và cuộc thi.</p><a href="dang-phat-trien.php?feature=co-hoi">Khám phá →</a></div></article><article><img src="assets/images/figma/icon-event.svg" alt=""><div><h3>Sự kiện</h3><p>Hội thảo, workshop và hoạt động nổi bật.</p><a href="dang-phat-trien.php?feature=su-kien">Khám phá →</a></div></article><article><img src="assets/images/figma/icon-impact.svg" alt=""><div><h3>Impact Box</h3><p>Dự án, sáng kiến và câu chuyện tác động.</p><a href="dang-phat-trien.php?feature=impact-box">Khám phá →</a></div></article></div></section>
